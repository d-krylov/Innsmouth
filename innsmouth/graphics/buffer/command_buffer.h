...
public:
  ...
  void CommandBindVertexBuffer(const Buffer &buffer, std::size_t offset = 0);
  void CommandBindIndexBuffer(const Buffer &buffer, std::size_t offset = 0, bool is_short_index = false);
  ...
  void CommandImageMemoryBarrier(const VkImage &image, ImageLayout from_layout, ImageLayout to_layout, PipelineStage from_stage,
                                 PipelineStage to_stage, const VkImageSubresourceRange &range);

  void CommandCopyBufferToImage(const Buffer &buffer, const Image &image, uint32_t mip_level, uint32_t base_layer = 0, uint32_t layers = 1,
                                uint64_t buffer_offset = 0, int32_t image_offset_x = 0, int32_t image_offset_y = 0,
                                int32_t image_offset_z = 0);

  void CommandPushDescriptorSet(const VkPipelineLayout layout, uint32_t set_number, uint32_t binding, const Image &image);

  void CommandPushDescriptorSet(const VkPipelineLayout layout, uint32_t set_number, uint32_t binding, const Buffer &buffer, uint64_t offset,
                                uint64_t size);

  template <typename T> void CommandPushConstants(const VkPipelineLayout layout, ShaderStage stage, const T &data, uint32_t offset = 0);
