...
template <typename T> void Buffer::SetData(std::span<const T> data, std::size_t offset) {
  GraphicsAllocator::Get()->CopyMemoryToAllocation(std::as_bytes(data), vma_allocation_, offset * sizeof(T));
}
